...
double TorusModelFit::optimize_function(double* gradient) {
	...
	double dHdMs[N*N];
	double A[30] = {0};
	double dHdSns[N*N][30];
	double dHdbs_mean = 0;
	double dHdMs_mean = 0;
	double dHdSns_mean[30] = {0};

	for(int i = 0; i < N; i++) {
		for(int j = 0; j < N; j++) {
			//double theta1_toy = M_PI * i / N / scale_angles;
			//double theta2_toy = M_PI * j / N / scale_angles;
			double theta1_toy = pow(10, ((double)i)/N*4-4) * 2 * M_PI;
			double theta2_toy = pow(10, ((double)i)/N*4-4) * 2 * M_PI;
			...
			*/
			...
			for(int k = 0; k < ns; k++) {
				...
				double d = 1/((x+x0) * (log(1+x0) - log(x0))) / 2 / M_PI;
				double dJ1_toydSn = mesh.eval(A, u);// * d;
				A[k] = 0;
				double dJ2_toydSn = 0;
#else
				double cos_ = cos((1+k) * theta1_toy);
				double dJ1_toydSn = 2 * (1+k) * cos_;
				double dJ2_toydSn = 0;
#endif
				...
				dHdSns[i+j*N][k] = dEkindSn + dEpotdSn;
				dHdSns_mean[k] += dHdSns[i+j*N][k]/(N*N);
#else
				for(int l = 0; l < ns; l++) {
					if((k > 0) || (l > 0)) {
#ifdef DO_FOURIER
						...
						double dJ1_toydSn = (*shapes[m])(theta1_toy*scale, theta2_toy*scale);
						double dJ2_toydSn = (*shapes[m])(theta1_toy*scale, theta2_toy*scale);
#endif
						//if(m == 0)
						//	return dJ1_toydSn;
	...
	for(int i = 0; i < N; i++) {
		for(int j = 0; j < N; j++) {
			if(gradient) {
				...
				for(int k = 0; k < ns; k++) {
#ifdef DO_ONE_D
					gradient[2+k] += 2 * (energy_mean-energies[i+j*N]) * (dHdSns_mean[k]-dHdSns[i+j*N][k]) / (N*N);
#else
					for(int l = 0; l < ns; l++) {
						if((k > 0) || (l > 0)) {
...
double TorusModelFit::energyChisq() {
	...
	for(int i = 0; i < N; i++) {
		for(int j = 0; j < N; j++) {
			...
			//cout << Epot << " " << Ekin << " r=" << r << " J1_toy=" << J1_toy << " theta1_toy=" << theta1_toy << " " << endl;
			energy_mean += energies[i+j*N] / (N*N); 
		}
	}
	double chisq = 0;
...
class MinimizerNLoptTest {
	...
	public:
		...
		template<class... Ts>
		void fill_initial(double* x, double value, Ts... tail) {
			x[0] = value;
			fill_initial(x+1, tail...);
		}
		void fill_initial(double* x, double value) {
			x[0] = value;
		}
		template<class... Ts>
		void optimize(int n, int Ndim, Ts... initial_values) {
			//const int N = sizeof...(Ts);
			nlopt_opt opt;
			//opt = nlopt_create(NLOPT_LD_MMA, N);
			opt = nlopt_create(NLOPT_LD_LBFGS, Ndim);
			...
			double* lower_bounds = new double[Ndim];
			double* upper_bounds = new double[Ndim];
			lower_bounds[0] = 0.1;
			lower_bounds[1] = 1;
...
void fit(TorusModelFit* fit, int steps, int n, double s) {
	//ad::testtorus();
	//ad::test();
...
//double J2 = atof(argv[5]);
	...
	//double theta2 = 0.2;
	//double scale = 1.;
	TorusModelIsochrone* toy = fit->toy_torus;
	int ns = fit->ns;
	double J1 = fit->J1;
	double J2 = fit->J2;
	...
	cout << "energy misfit " << fit->energyChisq() << endl;
	...
	for(int i = 0; i < ns; i++) {
		...
		chisq2 = fit->energyChisq();
		fit->S[i] -= dSn;
		cout << "dchisqdSn[1]= " << ((chisq2-chisq1)/dSn) << " " << (chisq2-chisq1) << endl;
	}
	...
	//int n = atoi(n);
#ifdef DO_SN
	o.optimize(n, 2+fit->ns, toy->scale, log(toy->M));
	//o.optimize<2+1>(n, toy->scale, log(toy->M), 0.);
#else
	o.optimize(n, 2, toy->scale, log(toy->M));
#endif
	...
	fit->toy_torus->get(J1, J2, 0.1, 0.1, H, Ekin, Epot, r);*/
}
...
using namespace gd::torus;
void py_export_torus_spherical() {
	def("torus_fit", torus::fit);
	class_<TorusModelFit >("TorusModelFit", init<Profile*, TorusModelIsochrone*, double, double, int>())
		.def("set", (&TorusModelFit::set))
		.def("getvr", (&TorusModelFit::getvr))
		.def("getvr", (&TorusModelFit::getvr_))
		.def("getr", (&TorusModelFit::getr))
		.def("getr", (&TorusModelFit::getr_))
		.def("getE", (&TorusModelFit::getE))
		.def("getEkin", (&TorusModelFit::getEkin))
		.def("getEpot", (&TorusModelFit::getEpot))
		.def("getJr_toy", (&TorusModelFit::getJr_toy))
		;
	class_<TorusModelIsochrone>("TorusModelIsochrone", init<double, double, double>())
		;
