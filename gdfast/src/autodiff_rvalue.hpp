...
V<T, index::index> var(T v, index i) {
...
public:
	bool evaluated;
	BinaryExpr(T1& t1, T2& t2) : t1(t1), t2(t2), v(-2), evaluated(false) {
...
//*
template<class T1, class T2>
Sum<typename std::remove_reference<T1>::type, typename std::remove_reference<T2>::type> operator+(T1&& t1, T2&& t2) {
	typedef Sum<typename std::remove_reference<T1>::type, typename std::remove_reference<T2>::type> Tt;
	return Tt(t1, t2);
}
/**/
/*
...
Sum<T1, T2> operator+(T1& t1, T2& t2) {
	...
	std::remove_reference<int&>::type la(2);
	return Tt(t1, t2);
}*/
