...
class PhotometryFake(object):
	def __init__(self, sub_scope_true, storage_2d_losvd, solution_known, N=10000):
...
class KinematicsObservation(object):
	def __init__(self, storage_2d_losvd, observation, sigma_v = 2.01, v_index_clip=None):
		self.storage_2d_losvd = storage_2d_losvd
...
class KinematicsObservationForeground(object):
	...
	def load(self):
		...
		self.ratios = array(self.ratios) *1e6# * 0.25 # * 1e6
...
class Discrete(object):
	def __init__(self, kinematics, photometry, storage_2d_m0, storage_2d_m2, storage_2d_m4, binned_data_m2, binned_data_m4, storage_3d, modelpath, schwsetname, schwmodelname, postfix=""):
	...
	def solve(self, scope):
		...
		Nparams = self.kinematics.pmatrixRN.shape[0]
		x = ones(Nparams) * 1.
		x /= sum(x)
