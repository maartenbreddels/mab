...
class BinaryNode(object):
	def __init__(self, grid, domain, dimension, level=1):
		...
		self.domain = domain
		self.childNodes = []
		self.grid = grid.create_grid(domain)
	...
	def calculate_volumes(self, parentvolume):
		volume = parentvolume / len(self.corners)
...
class BinaryGrid(object):
	...
	def integrate(self):
		return sum([self.volumes[p]*self.values[p] for p in self.points])
		
	def calculate_volumes(self):
		self.volumes = {}
		self.rootNode.calculate_volumes(1.0)
	
	def add_volume(self, point):
		self.volumes[point] += calculate_volumes
...
class BinaryGridTri(BinaryGrid):
	...
	def create_grid(self):
		return basis.BasisTriLevels(0., 1., 0)
