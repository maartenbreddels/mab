...
class SubScope(object):
	def __init__(self, scope, globals=[], includes=[], configurations=[], inherit_configurations=True):
		self.scope = scope
		self.globals = globals
		self.includes = includes
		self.configurations = configurations
		self.inherit_configurations = inherit_configurations
		
	def load(self):
		self.subscope = self.scope.clone()
		if self.inherit_configurations:
			self.subscope.configurations.extend(self.configurations)
		else:
			self.subscope.configurations = list(self.configurations)
			
		#print "#" * 80 * 2
		#print self.subscope.configurations
		#print self.globals
		self.subscope.globals.extend(self.globals)
		#self.subscope.globals = list(self.globals)
		self.subscope.readfiles(*(list(self.scope.extrafilenames)+self.includes))
		#print self.scope
...
class IniScope(object):
	...
	def clone(self):
		clone = IniScope(self.filename, aliases=self.aliases, globals=list(self.globals))
		...
		clone.extrafilenames = self.extrafilenames
		return clone
